...
namespace Aircompany
{
    public class Airport
    {
        ...
        public PassengerPlane GetPassengerPlaneWithMaxPassengersCapacity()
        {
            ...
            var maxCapacity = passengerPlanes.Max(y => y._maxLoadCapacity);
            return passengerPlanes.FirstOrDefault(x => x._maxLoadCapacity == maxCapacity);           
        }

        public List<MilitaryPlane> GetTransportMilitaryPlanes()
        {
            List<MilitaryPlane> militaryPlanes = GetMilitaryPlanes();
            return militaryPlanes.Where(x => x.PlaneTypeIs() == MilitaryType.TRANSPORT).ToList(); ;
        }

        public Airport SortByMaxDistance() => new Airport(_planes.OrderBy(w => w.MAXFlightDistance()));
        public Airport SortByMaxSpeed() => new Airport(_planes.OrderBy(w => w.GetMS()));
